...
json_parse_ex(const void *src, size_t src_size, size_t flags_bitset,
    ...
    input_error =
        json_get_value_size(&state, (int)(json_parse_flags_allow_global_object &
                                          state.flags_bitset));
    ...
    *data++ = '['; /* open the array. */
    ...
    if (0 < array->length) {
        ...
        for (element = array->start; json_null != element;
            ...
            data = json_write_pretty_value(element->value, depth + 1, indent,
                                           newline, data);
...
void *json_write_pretty(const struct json_value_s *value, const char *indent,
    ...
    data = size == 0 ? json_null : (char *)malloc(size);
